...
$img = '#(http://[^\s]+(?=\.(jpe?g|png|gif)))#i';
$yt = '/(?:http:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/(?:watch\?v=)?(.+)/g';

if(preg_match($img, $str)) {
	$str = preg_replace($img, "<a href='$1' target='_blank'><img src='$1' width='290px' height='auto' style='margin: 5px 0 0 0;'></a>", $str);
	echo "test";
} else if(preg_match($yt, $str) {
	$str = preg_replace($yt, '<span data-link="$1" class="yt-btn" style="background: url(http://pyxl.crater.uberspace.de/fume/img/play_fume.png) no-repeat; width: 284px; margin: 10px 0 10px 110px; height: 49px; cursor: pointer;"></span>', $str);
} else {
	$str = preg_replace($link, "<a target='_blank' href='$1'>$1</a>", $str);
}


$str2 = preg_replace_callback('#(?:https?://\S+)|(?:www.\S+)|(?:\S+\.\S+)#', function($arr)
{
      ...
      && isset($url['query']))
    {
        ...
        return sprintf('<span data-link="%s" class="yt-btn" style="background: url(http://pyxl.crater.uberspace.de/fume/img/play_fume.png) no-repeat; cursor: pointer; width: 284px; margin: 10px 0 10px 110px; height: 49px;"></span>', $query['v']);
    }
    //links
